...
function FlashCardForm({ deckId, deck }) {
  ...
  useEffect(() => {
    ...
    async function fetchData(deckId) {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/v1/deck/${deckId}`,
        {
          method: "GET",
          headers: {
  ...
  async function handleSaveCard(e) {
    e.preventDefault();
    try {
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/v1/flashcard`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async function handleDelete(id) {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/v1/flashcard/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
